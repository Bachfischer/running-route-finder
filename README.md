...
Each search looks for a nearby mapped park using HeiGIT's park POIs and requests a walking route through it, then compares four seeded `foot-walking` round trips. A further request may correct an inaccurate round-trip distance. Routes through parks are preferred when their length is reasonable; candidates also favor measured green and quiet segments and mapped paths. ORS ratings describe mapped route segments and paths do not necessarily lie inside parks; the service does not count traffic lights or promise a stop-free run. Check local signs and conditions. API requests have bounded bodies and provider timeouts; failed providers produce readable errors without disclosing the key. `GET /api/health` checks the application only.
