...
Core coverage includes `lib/routing.ts`, `scenery.ts`, `route-search.ts`, `providers.ts`, `http.ts`, `gpx.ts` and `errors.ts`. All maintained library files are included; unused starter utilities have been removed. Gates are 90% lines, 90% functions and 80% branches. Coverage is not a claim of correctness; the important assertions include missing graph links, no fabricated river/building connectors, conditional/private foot access, barriers, pedestrian one-way behavior, no-loop networks, geographic boundaries, response-size cancellation, adaptive dense-area retries preserving 10 km, provider failures and simultaneous requests from multiple devices.
