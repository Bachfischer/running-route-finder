...
| Browser | `pnpm test:ui` after build/config | Playwright desktop and mobile Chromium; 18 scenarios each, screenshots and traces on failure |
...
During controlled-browser testing, the GPX download-event waiter timed out. GPX serialization is unit tested; the real download assertion remains part of the Playwright CI suite and must be verified there.
