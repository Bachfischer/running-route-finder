...
For production routing, configure `ORS_API_KEY` as a sensitive Vercel environment variable in Preview and Production and redeploy both. The key must stay server-side. The app requests at most four managed foot-walking loops per search (green and quiet preferences, mapped stairs avoided), which consumes the provider's daily quota. The ORS quality percentages are provider ratings, not park-boundary measurements; no signal/crossing count is available. Confirm a live 10 km Munich route and GPX on the deployed commit before announcing availability. A green CI build alone is not proof that the provider accepts the key or that the production alias points at the new version.
