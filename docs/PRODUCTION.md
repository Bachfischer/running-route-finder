...
- A connection failure or timeout at the default Overpass host gets one sequential attempt at `overpass.private.coffee`. Each provider gets up to 40 seconds, within an 80-second area budget capped by the remaining 110-second search deadline. HTTP errors (including throttling), cancelled requests, malformed data and custom-provider failures do not trigger failover.
...
For production routing, configure `ORS_API_KEY` as a sensitive Vercel environment variable in Preview and Production and redeploy both. The key must stay server-side. The app requests at most five managed foot-walking loops per search (four green and quiet alternatives, at most one length adjustment, mapped stairs avoided), which consumes the provider's daily quota. The ORS quality percentages are provider ratings, not park-boundary measurements; no signal/crossing count is available. Confirm a live 10 km Munich route and GPX on the deployed commit before announcing availability. A green CI build alone is not proof that the provider accepts the key or that the production alias points at the new version.

Search and route responses carry `X-Request-ID`. Application logs contain request ID, operation, HTTP status and duration. Map-provider events additionally contain a fixed provider label, outcome category and duration, correlated through the hosting invocation. They omit provider URLs, query strings, coordinates, addresses, client IPs and raw exceptions. Hosting access logs are controlled separately in Vercel. Start alerts for sustained 5xx responses, increased route latency, and unusual 429 volume. Inspect both provider availability and CPU duration before changing timeout limits.
