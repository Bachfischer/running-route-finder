...
`munich-10km.json.gz` is an OpenStreetMap extract obtained via Overpass on 2026-09-13. It contains 77,411 elements in the first northward search area for a 10 km run from Odeonsplatz (48.142, 11.577). The network and routing/access/barrier/traffic-signal tags and green-space geometry are preserved; unrelated tags have been removed. It is not a route fabricated for the tests.
...
The exact query is `munich-query.overpass`. `munich-result.json` is the computed result used by deterministic browser tests, also ODbL-derived. It is not loaded by the application. The park-focused algorithm finds 9.974 km, 74.3% green-space coverage, 91.9% paths/tracks, 4.6% repeated distance and zero mapped signal encounters.
